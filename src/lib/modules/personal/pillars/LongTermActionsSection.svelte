<script lang="ts">
        ...
        try {
            ...
            if (response.status === 200 && response.data) {
                ...
                if (categoryInfo.long_actions && categoryInfo.long_actions.length > 0) {
                    const nonEmptyActions = categoryInfo.long_actions
    ...
    function handleBlur() {
        ...
        console.log('LongTermActionsSection Blur event - autosave disabled');
    }
...
<div class="flex flex-col">
    <div class="flex items-center gap-2">
        <h2 class="text-2xl font-medium text-alineados-gray-900">Acciones Futuro - Largo plazo</h2>
        <Tooltip
            open={false}
            messages={[
