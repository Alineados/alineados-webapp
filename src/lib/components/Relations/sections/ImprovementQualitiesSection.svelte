<script lang="ts">
    ...
    // Estado local para cualidades por mejorar
    let items = $state([
    ...
    $effect(() => {
        if (relation.improvement_qualities && relation.improvement_qualities.length > 0) {
            const qualityItems = relation.improvement_qualities.map(quality => ({
                id: nanoid(),
                description: quality,
                prominent: false,
                daily: false
    ...
    function syncChanges() {
        ...
        relation.improvement_qualities = itemsWithContent.map(item => item.description);
        // Autosave
        debouncedSaveRelation({ improvement_qualities: relation.improvement_qualities });
    }
...
<div class="flex flex-col">
    <div class="flex items-center gap-2">
        ...
        <h2 class="text-2xl font-medium text-alineados-gray-900">Cualidades por mejorar</h2>
    </div>
