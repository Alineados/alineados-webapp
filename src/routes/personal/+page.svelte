    ...
    <div class="mt-8 grid grid-cols-1 gap-8 sm:grid-cols-2">
        {#each cards as card}
            <button 
                ...
                class="group relative flex h-64 flex-col items-center justify-center rounded-xl bg-[#E8F5EC] p-8 transition-all hover:shadow-lg"
            >
                {#if isLoading && loadingCardHref === card.href}
                    <div class="absolute inset-0 flex items-center justify-center rounded-xl bg-[#E8F5EC]/90 backdrop-blur-sm">
                        <Loader />
                    </div>
                {/if}
